...
@Singleton
public class LoadingDwhDBOperator extends EniqDBOperator {

	private static Object String;
	Logger logger = LoggerFactory.getLogger(LoadingDwhDBOperator.class);
	...
	public HashMap<String, String> verifyDBCommand3(String query, Map url) {
		...
		if (resultOutput.iterator().hasNext()) {
			for (String output : TestDataSourceFormatter.format(resultOutput).trim().split("\n")) {
				if (!output.contains("*")|| output.contains("not found") ) {
					if (output.contains("DUPLICATE")|| output.contains("not found")) {
